...
export default function CheckInPage() {
  ...
  const [guests, setGuests] = React.useState<any[] | null>(null);
  ...
  const refresh = React.useCallback(async () => {
    ...
    const [{ data: eventData }, { data: guestsData }] = await Promise.all([
      supabase.from('events').select('*').eq('id', safeEventId).eq('owner_id', user.id).maybeSingle(),
      supabase.from('guests').select('*').eq('event_id', safeEventId).order('created_at', { ascending: false }),
    ]);
    setEvent(eventData);
  ...
  const handleManualCheckIn = async (guestId: string, currentStatus: boolean) => {
    try {
      const values = currentStatus ? { checked_in: false, checked_in_at: null } : { checked_in: true, checked_in_at: new Date().toISOString() };
      const { error } = await supabase.from('guests').update(values).eq('id', guestId).eq('event_id', safeEventId);
  ...
  const resolveTokenCheckIn = async (token: string) => {
    ...
    const guest = guests.find((g: any) => g.qr_token === normalized);
    if (!guest) {
      setQrToken('');
    ...
    await handleManualCheckIn(guest.id, false);
    setQrToken('');
  };
  ...
  if (!event || !guests) return <div className="p-8 text-text-muted font-medium">Loading...</div>;

  const filteredGuests = guests.filter((g: any) => g.full_name.toLowerCase().includes(searchQuery.toLowerCase()) || g.email?.toLowerCase().includes(searchQuery.toLowerCase()));
