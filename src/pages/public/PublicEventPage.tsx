...
const safeFormatTime = (dateStr: string, endDateStr?: string, fallback = '') => {
  try {
    ...
    if (endDateStr) {
      const e = new Date(endDateStr);
      if (!isNaN(e.getTime())) return `${start} - ${e.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}`;
    }
    return start;
  } catch { return fallback; }
};
...
export default function PublicEventPage() {
  ...
  const catalogTemplate = TEMPLATE_CATALOG.find(t => t.id === event.template_id);
  const themeColor: string = event.theme_color || catalogTemplate?.color || '#18181B';
  const typographyPreset: string = event.typography_preset || 'modern';
  ...
  return (
    <div className="antialiased overflow-x-hidden min-h-screen flex flex-col bg-white" style={{ fontFamily }}>
      <div className="relative min-h-[60vh] flex items-end justify-center overflow-hidden" style={{ backgroundColor: themeColor }}>
        {event.cover_image_url && (
          <div className="absolute inset-0 z-0">
            <img alt={event.title} className="w-full h-full object-cover" src={event.cover_image_url} style={{ opacity: 0.5 }} />
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto pb-20 pt-32">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <p className="text-sm md:text-base uppercase tracking-widest mb-6 font-bold" style={{ color: accentText, opacity: 0.75 }}>You are invited</p>
            <h1 className="text-5xl md:text-7xl lg:text-8xl mb-8 leading-tight font-bold" style={{ color: accentText }}>{event.title}</h1>
            <p className="text-xl md:text-2xl font-medium" style={{ color: accentText, opacity: 0.9 }}>{safeFormatShort(event.starts_at, event.ends_at)}</p>
          </motion.div>
        </div>
      </div>
      <div className="flex-1 relative py-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-white border border-gray-100 rounded-3xl shadow-2xl p-8 md:p-12 text-center relative z-10 -mt-16">
          <h2 className="text-3xl md:text-4xl mb-12 font-bold text-gray-900">The Details</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
            <div><span className="material-symbols-outlined text-4xl mb-4" style={{ color: themeColor, opacity: 0.8 }}>schedule</span><h3 className="text-sm font-bold uppercase tracking-widest text-gray-400 mb-2">When</h3><p className="text-xl font-semibold text-gray-900 mb-1">{safeFormat(event.starts_at, event.ends_at)}</p><p className="text-gray-500">{safeFormatTime(event.starts_at, event.ends_at)}</p></div>
